...
export function buildContinueUserPrompt(input: ContinueAnalysisInput): string {
  ...
  lines.push(
    ...
    "decisionAuthority пересчитай для текущего разбора; не копируй предыдущий, если он больше не относится. Если текущий пробел — определяющий факт, не ставь owner=principal лишь потому что решение потом может быть важным. Если принципал уже утвердил релевантное суждение — не спрашивай его снова.",
